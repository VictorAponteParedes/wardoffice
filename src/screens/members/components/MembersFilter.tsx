...
interface MembersFilterProps {
    ...
    callingFilter: string;
    onCallingFilterChange: (value: string) => void;
    onSearch: () => void;
}
    ...
    return (
        <div className="bg-white p-4 rounded-xl shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between mb-6">
            <div className="flex flex-1 gap-4 w-full md:w-auto">
                ...
                <div className="flex-1 flex items-center gap-2">
                    <label className="text-sm font-bold text-gray-700 whitespace-nowrap">Llamamiento:</label>
                    <input
                        type="text"
                        value={callingFilter}
                        onChange={(e) => onCallingFilterChange(e.target.value)}
                        placeholder="Filtrar por llamamiento..."
                        className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500 transition-colors"
                    />
                </div>
            </div>
